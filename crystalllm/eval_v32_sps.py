"""
eval_v32_sps.py — v32 SpS 评估 (100M 扩散 drafter + 32L AR verifier)
"""
...
# v32 verifier (32L)
ckpt_v = torch.load("v32_decoder.pt", map_location="cuda", weights_only=False)
cfg = ckpt_v["config"]
...
verifier.eval()
P(f"v32 verifier: {sum(p.numel() for p in verifier.parameters())/1e6:.2f}M")
...
# v32 100M drafter
ckpt_d = torch.load("v32_diff_drafter.pt", map_location="cuda", weights_only=False)
dcfg = ckpt_d["config"]
...
tok_emb.eval()
P(f"v32 drafter: {sum(p.numel() for p in drafter.parameters())/1e6:.2f}M + tok_emb: {sum(p.numel() for p in tok_emb.parameters())/1e6:.2f}M")
...
@torch.no_grad()
def gen_v32_sps(n_ar=100, K=8, n_diff_steps=5):
...
P("\n=== 速度对比 ===")
t_v32_ar = bench(gen_v32_ar, label="v32 AR baseline")
t_v32_sps = bench(lambda: gen_v32_sps(n_ar=100, K=8), label="v32 SpS K=8")
P(f"\nv32 SpS 速度: {t_v32_sps:.0f} ms")
P(f"  vs v32 AR: {t_v32_ar:.0f} ms, 加速 {t_v32_ar/t_v32_sps:.2f}x")
P(f"  vs v28.5 AR (544ms): {544/t_v32_sps:.2f}x")
P(f"  vs v31 SpS (206ms): {206/t_v32_sps:.2f}x")

# 接受率
P("\n=== 接受率 ===")
total_d = 0; total_a = 0
for trial in range(10):
    cur, n_rounds, drafted, accepted = gen_v32_sps(n_ar=100, K=8)
    total_d += drafted; total_a += accepted
acc_rate = total_a / total_d if total_d > 0 else 0
P(f"  接受: {total_a}/{total_d} = {acc_rate*100:.1f}%")

# 生成质量
P("\n=== 生成质量 ===")
cur_ar = gen_v32_ar(n_ar=100)
cur_sps, n_rounds, _, _ = gen_v32_sps(n_ar=100, K=8)
P(f"v32 AR:  {repr(''.join([itos.get(t, '?') for t in cur_ar[:60]]))}")
P(f"v32 SpS: {repr(''.join([itos.get(t, '?') for t in cur_sps[:60]]))}")
P(f"v32 SpS rounds: {n_rounds}")
...
times = []
for _ in range(10):
    torch.cuda.synchronize(); t0 = time.time(); time_prior(); torch.cuda.synchronize()
    times.append((time.time() - t0) * 1000)
...
times = []
for _ in range(10):
    torch.cuda.synchronize(); t0 = time.time(); time_drafter(); torch.cuda.synchronize()
    times.append((time.time() - t0) * 1000)
P(f"  扩散 drafter (K=8, 100M): {np.mean(times):.2f} ms")
...
times = []
for _ in range(10):
    torch.cuda.synchronize(); t0 = time.time(); time_verifier(); torch.cuda.synchronize()
    times.append((time.time() - t0) * 1000)
P(f"  verifier 1 forward (32L, K=8): {np.mean(times):.2f} ms")

results = {
    "v32_ar_ms": t_v32_ar, "v32_sps_ms": t_v32_sps,
    "speedup_vs_v32_ar": t_v32_ar / t_v32_sps,
    "speedup_vs_v28_5_ar": 544 / t_v32_sps,
    "speedup_vs_v31_sps": 206 / t_v32_sps,
...
with open("v32_sps_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2)
P(f"\nSaved: v32_sps_results.json")