...
"""
train_v23_encoder.py — v23-1: 256 维 z encoder (无主题对齐)
...
复用 v22 encoder 架构:
  ...
  Mini Decoder (4L × 512 × 8, 30M) — 临时重建监督
    - 训完丢弃, v23 端到端用 v22a 500M decoder warm-start

数据: v23_train.parquet (6317) + v23_val.parquet (332)
v23 来源: 2305 sessions 切窗 5000/5000 → 6649 样本
"""
...
P("=== v23-1 256D Encoder (no theme) STARTUP ===")
DATA = Path("data/processed")

# 优先用 v23 vocab, 退化到原 vocab
vocab_file = DATA / "char_vocab_v23.json" if (DATA / "char_vocab_v23.json").exists() else DATA / "char_vocab.json"
...
P(f"Vocab {V} (from {vocab_file.name}) | BOS={BOS_ID} PAD={PAD_ID} EOS={EOS_ID}")
...
class Encoder(nn.Module):
    """v23 encoder: 12L × 768 × 12 bi-attn, 输出 256 维 mu/logvar, 无主题头."""
    def __init__(s):
        super().__init__()
...
P(f"\nEncoder: {n_enc/1e6:.2f}M (12L×768×12, output D_Z={D_Z}, no theme)")
...
P(f"\n=== train {STEPS} steps (encoder+mini_dec, no theme) ===")
P(f"  L = {W_RECON}*L_recon + {W_KL}*β*L_KL (no theme)")
t0 = time.time()
log = []
...
torch.save({"encoder": encoder.state_dict(),
            "config": {"V": V, "T": T, "D_Z": D_Z,
                       ...
                       "n_themes": 1, "no_theme": True}},
           SAVE)
P(f"\nEncoder saved: {SAVE}")

with open("v23_encoder_train_log.json", "w", encoding="utf-8") as f:
    json.dump({"log": log, "config": {"STEPS": STEPS, "D_Z": D_Z,
                                        ...
                                        "arch": "v23-256D-encoder-no-theme",
                                        "data": "v23 sliding window 5000/5000"}}, f, indent=2)
P(f"Log saved: v23_encoder_train_log.json")
