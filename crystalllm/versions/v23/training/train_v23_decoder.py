...
"""
train_v23_decoder.py — v23-4: 500M decoder warm-start (在 v23 z 上)

复用 v22a decoder 权重 (D_Z 都是 256):
  - z_to_emb: 256→1280 直接复用 (列数一致)
  - 其他层权重全部复制

训练数据: v23 (6317 train + 332 val), z 用 cached_v23_z.npz
训练: 4000 步 (vs v22a 2000, 数据量 3.3x, 多训一些)
...
P("=== v23-4 500M Decoder Warm-Start (v23 z, 6317 train, 用 v22 vocab 完全 warm-start) ===")
DATA = Path("data/processed")
# 关键修复: 用 v22 vocab (2261) 让 tok.weight/head.weight 形状匹配, 完全 warm-start
...
P(f"Vocab {V} (from char_vocab.json, 与 v22 一致) | BOS={BOS_ID} PAD={PAD_ID} EOS={EOS_ID}")
# 验证 v23 数据用 v22 stoi 没有 OOV
df_train = pd.read_parquet(DATA / "v23_train.parquet")
df_val = pd.read_parquet(DATA / "v23_val.parquet")
all_chars = set()
for t in df_train["text"].tolist() + df_val["text"].tolist():
    all_chars.update(t)
v22_chars = set(stoi.keys())
v23_only = all_chars - v22_chars
P(f"v23 数据用 v22 stoi: 字符集 {len(all_chars)}, v22 包含 {len(all_chars & v22_chars)}, v23-only {len(v23_only)}: {list(v23_only)[:5]}")
if v23_only:
    P(f"  ⚠️ v23-only 字符将映射到 <unk>=0, 训练时忽略")

train_texts = df_train["text"].tolist()
val_texts = df_val["text"].tolist()
P(f"v23 train: {len(train_texts)} | val: {len(val_texts)}")

# ===== 配置 =====
B, T = 16, 128
D_Z = 256  # v23 (与 v22a 一致)
DEC_LAYER, DEC_HEAD, DEC_EMBD = 24, 20, 1280
LR, STEPS = 1e-4, 4000   # v22a 是 2000, 数据多训到 4000
EVAL_EVERY = 250
...
class Decoder(nn.Module):
    """v23 decoder: 24L×1280×20, z 256 维输入 (与 v22a 相同架构)."""
...
# ===== Warm-start 加载 v22 decoder 权重 =====
P("\n=== Warm-start: 加载 v22a decoder 权重 ===")
ckpt_v22 = torch.load("v22_decoder.pt", map_location="cuda", weights_only=False)
v22_state = ckpt_v22["decoder"]
v22_cfg = ckpt_v22["config"]
D_Z_V22 = v22_cfg["D_Z"]
P(f"v22 decoder: D_Z={D_Z_V22}, layers={v22_cfg['DEC_LAYER']}")
...
for k, v in v22_state.items():
    if k in new_state:
        if v.shape == new_state[k].shape:
            new_state[k] = v
        elif k == "z_to_emb.weight" and v.shape == (DEC_EMBD, D_Z_V22):
            # D_Z 都是 256, 应该形状一致, 不需要扩列
            P(f"  z_to_emb.weight: 形状匹配 {v.shape}")
            new_state[k] = v
        else:
...
P(f"\nv23 decoder: {n_dec/1e6:.2f}M (warm-started from v22a)")
...
P(f"Loaded v23 cached z: train {train_z_cache.shape} | val {val_z_cache.shape}")
...
for step in range(STEPS):
    ...
    for i in ix:
        ...
        x_chunks.append([stoi.get(c, 0) for c in chunk])  # v23-only 字符 → <pad>=0
...
torch.save({"decoder": decoder.state_dict(),
            "config": {"V": V, "T": T, "D_Z": D_Z,
                       ...
                       "warm_start_from": "v22_decoder"}},
           SAVE)
P(f"\nModel saved: {SAVE}")

with open("v23_decoder_train_log.json", "w", encoding="utf-8") as f:
    json.dump({"log": log, "config": {"STEPS": STEPS, "D_Z": D_Z,
                                        ...
                                        "warm_start_from": "v22_500M_decoder",
                                        "n_train": len(train_texts), "n_val": len(val_texts),
                                        "arch": "v23-500M-BAD-decoder-256D-warm-start"}}, f, indent=2)
P(f"Log saved: v23_decoder_train_log.json")
