...
"""
train_v23_diffusion.py — v23-3: 256 维扩散先验 (在 v23 z 上)

复用 v22 prior 架构 (D_Z=256, D_HID=512, 6 ResBlock, ~6.58M 参数).
数据: v23 cached_z (6317 train + 332 val).
"""
...
P("=== v23-3 256D Diffusion Prior STARTUP ===")
D_Z = 256
D_HID = 512
N_LAYER = 6
LR, STEPS = 1e-3, 4000
...
P(f"v23 z: train {train_z.shape} | val {val_z.shape}")
...
with open("v23_diffusion_log.json", "w", encoding="utf-8") as f:
    json.dump({"log": log, "config": {"STEPS": STEPS, "D_Z": D_Z, "D_HID": D_HID,
                                        ...
                                        "arch": "v23-256D-CFM-diffusion-prior"}}, f, indent=2)
P(f"Log saved: v23_diffusion_log.json")
