...
"""
extract_v23_z.py — v23-2: 提取 256 维 z 缓存

用训好的 v23 encoder 提取 6317 train + 332 val 的 256 维 z
v23 数据集: 2305 sessions 切窗 5000/5000 → 6649 样本
"""
...
print(f"vocab from {vocab_file.name}: V={V}")

# 加载 v23 encoder
ckpt = torch.load("v23_encoder.pt", map_location="cuda", weights_only=False)
cfg = ckpt["config"]
T, D_Z = cfg["T"], cfg["D_Z"]
ENC_LAYER, ENC_HEAD, ENC_EMBD = cfg["ENC_LAYER"], cfg["ENC_HEAD"], cfg["ENC_EMBD"]
print(f"v23 encoder: {ENC_LAYER}L × {ENC_EMBD} × {ENC_HEAD} | D_Z={D_Z}")
...
print(f"v23 train: {len(train_texts)} | val: {len(val_texts)}")
...
# 保存
SAVE = "cached_v23_z.npz"
np.savez(SAVE, train_z=train_z, val_z=val_z)
print(f"\nSaved: {SAVE}")
print(f"  train_z: {train_z.shape}")
print(f"  val_z: {val_z.shape}")
print(f"  train_z mean: {train_z.mean():.3f}, std: {train_z.std():.3f}")
print(f"  val_z   mean: {val_z.mean():.3f}, std: {val_z.std():.3f}")
print(f"\n=== 完成 ===")
