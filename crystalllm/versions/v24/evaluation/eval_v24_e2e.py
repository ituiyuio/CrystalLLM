...
"""
eval_v24_e2e.py — v24 端到端 PPL + 速度评估

v24 = 256 维 z (v24 encoder) + 256 维 prior (v24 diffusion) + 500M decoder (warm-start from v23)
无主题控制 (v22a 已证失败)
...
# v24 decoder 用 v22 vocab (2261), 修复 warm-start
...
print(f"vocab from char_vocab.json: V={V} (与 v23 一致)")


ckpt_dec = torch.load("v24_decoder.pt", map_location="cuda", weights_only=False)
cfg = ckpt_dec["config"]
T, D_Z = cfg["T"], cfg["D_Z"]
DEC_LAYER, DEC_HEAD, DEC_EMBD = cfg["DEC_LAYER"], cfg["DEC_HEAD"], cfg["DEC_EMBD"]
print(f"v24 decoder: {DEC_LAYER}L × {DEC_EMBD} × {DEC_HEAD}, D_Z={D_Z}")
...
print(f"v24 decoder params: {n_dec/1e6:.2f}M")
...
t_v24, _ = bench(gen_v24, label=f"v24 端到端 (5步扩散+{N_AR} AR)")
...
print(f"  v24  (475M, 256z):   e2e {ppl_diff:.2f}   | 19.3K train (3.1x v23, 10x v22a)")

print(f"\n=== 速度 KR1.3 ===")
print(f"  v24 端到端: {t_v24:.0f} ms")
print(f"  v22a 端到端: 847 ms (KR1.3 0.32x)")
print(f"  v23 端到端: 761 ms (KR1.3 0.286x)")
print(f"  500M AR baseline: 2665 ms")
print(f"  v24 vs AR: {t_v24/2665:.3f}x")

results = {
    "config": {"decoder_params_M": n_dec / 1e6, "D_Z": D_Z, "DEC_LAYER": DEC_LAYER,
               "n_train": 19307, "n_val": 1016, "vocab": V,
               "warm_start_from": "v23_decoder", "data": "v24 from raw_v23 24GB (swift code + agentic)"},
    ...
    "speed_ms": {"v24_e2e": t_v24},
    "comparison": {
        "v18_e2e_ppl": 17.71, "v20a_e2e_ppl": 13.05, "v21_e2e_ppl": 5.83,
        "v22a_e2e_ppl": 4.39, "v23_e2e_ppl": 3.37,
        "v21_speed_ms": 786, "v22a_speed_ms": 847, "v23_speed_ms": 761,
        "v21_train_samples": 1893, "v22a_train_samples": 1893,
        "v23_train_samples": 6317, "v24_train_samples": 19307,
    },
}
with open("v24_e2e.json", "w", encoding="utf-8") as f:
