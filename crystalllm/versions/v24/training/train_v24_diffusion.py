...
"""
train_v24_diffusion.py — v24-3: 256 维扩散先验 (在 v24 z 上)

复用 v22 prior 架构 (D_Z=256, D_HID=512, 6 ResBlock, ~6.58M).
数据: cached_v24_z.npz (19307 train + 1016 val).
"""
...
P("=== v24-3 256D Diffusion Prior STARTUP ===")
D_Z = 256
D_HID = 512
N_LAYER = 6
LR, STEPS = 1e-3, 4000
...
P(f"v24 z: train {train_z.shape} | val {val_z.shape}")
...
with open("v24_diffusion_log.json", "w", encoding="utf-8") as f:
    json.dump({"log": log, "config": {"STEPS": STEPS, "D_Z": D_Z, "D_HID": D_HID,
                                        "N_LAYER": N_LAYER, "N_SAMPLE_STEPS": N_SAMPLE_STEPS,
                                        "params_M": n_params/1e6,
                                        "n_train_z": len(train_z), "n_val_z": len(val_z),
                                        "arch": "v24-256D-CFM-diffusion-prior"}}, f, indent=2)
P(f"Log saved: v24_diffusion_log.json")
