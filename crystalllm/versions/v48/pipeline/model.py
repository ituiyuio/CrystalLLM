"""model.py — v48 Phase 2: 1.2B From-Scratch + Extended Sparse Attention

架构 (~1.2B active params):
  Hidden dim: 2048
  Layers:     24
  Heads:      16 (head_dim 128)
  Dense FFN:  2048 -> 8192 -> 2048     (variant A)
  MoE FFN:    8 experts x (2048->4096->2048), Top-2 (variants B, C)
  T (sequence): 1024 tokens (64 blocks of 16)
  Sparse attn (variant C):
    - z_emb / BOS positions: 全局可见
    - x in block k: attend to (all z + x in same block + x in window ±2 blocks)
  ...
  Total positions (variant C): 18 + 63*17 = 1089

三个变体:
  A: dense FFN + dense attn + AR only
  B: MoE FFN + dense attn + AR only
  C: MoE FFN + per-block z + sparse attn (±2) + 0.5 L_AR + 0.5 L_diff
"""
...
DEC_LAYER = 24
DEC_HEAD = 16
HEAD_DIM = DEC_EMBD // DEC_HEAD  # 128
FFN_DIM_DENSE = 8192
FFN_DIM_EXPERT = 4096
N_EXPERTS = 8
TOP_K = 2
...
BLOCK_SIZE = 16
T_TOKEN = 1024
N_BLOCKS = T_TOKEN // BLOCK_SIZE  # 64
# Variant C total positions: 18 + 63*17 = 1089
V48_BLOCK_POS_LEN = (BLOCK_SIZE + 2) + (N_BLOCKS - 1) * (BLOCK_SIZE + 1)
# Variant A/B total positions: T + 2 = 1026
V48_FLAT_POS_LEN = T_TOKEN + 2

# Sparse attention: window (邻块数)
SPARSE_WINDOW = 2  # ±2 邻块 (vs v47 的 ±1)
...
def build_sparse_mask(T_total: int, n_blocks: int, block_size: int,
    ...
    i_block = pos_block.unsqueeze(1)
    j_block = pos_block.unsqueeze(0)
    i_type = pos_type.unsqueeze(1)
    j_type = pos_type.unsqueeze(0)

    i_is_x = (i_type == 2)
    j_is_z = (j_type == 0) | (j_type == 1)

    x_to_x = (~j_is_z) & ((j_block - i_block).abs() <= window)
    x_to_z = j_is_z
    z_to_all = ~i_is_x
...
class SparseAttention(nn.Module):
    ...
    def __init__(self, n_embd: int = DEC_EMBD, n_head: int = DEC_HEAD,
                 use_sparse: bool = False, max_T: int = V48_BLOCK_POS_LEN):
        super().__init__()
        self.nh = n_head
...
# Transformer Block
# ============================================================
class BlockCausalV48(nn.Module):
    def __init__(self, n_embd: int = DEC_EMBD, n_head: int = DEC_HEAD,
...
class MoEFFNV48(nn.Module):
    ...
    def __init__(self, n_embd: int = DEC_EMBD, ffn_dim: int = FFN_DIM_EXPERT,
        ...
        self.experts = nn.ModuleList([
            DenseFFNV48(n_embd, ffn_dim) for _ in range(n_experts)
        ])
...
class V48Decoder(nn.Module):
    def __init__(
        ...
        self.total_pos = (
            V48_BLOCK_POS_LEN if use_per_block_z else V48_FLAT_POS_LEN
        )
        ...
        self.blocks = nn.ModuleList([
            BlockCausalV48(n_embd, n_head, use_sparse_attn=use_sparse_attn)
            for _ in range(n_layer)
        ])
        for blk in self.blocks:
            if ffn_type == "dense":
                blk.set_ffn(DenseFFNV48(n_embd, ffn_dim_dense))
            elif ffn_type == "moe":
                blk.set_ffn(MoEFFNV48(n_embd, ffn_dim_expert, n_experts, top_k))
            else:
    ...
    def _forward_flat(self, z, x, mask_input):
        ...
        if self.use_grad_checkpoint and self.training:
            ...
            for blk in self.blocks:
                inp = checkpoint(blk, inp, use_reentrant=False)
        else:
            for blk in self.blocks:
                inp = blk(inp)
...
def ar_loss(decoder: V48Decoder, z, x, mask_input=None):
    logits, _ = decoder(z, x, mask_input=mask_input)
    return F.cross_entropy(logits.reshape(-1, decoder.V), x.reshape(-1))


def block_diffusion_loss(decoder: V48Decoder, z, x, mask_rate_range=(0.1, 0.5)):
