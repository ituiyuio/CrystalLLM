...
decoder.eval()
P(f"v28.5 params: {sum(p.numel() for p in decoder.parameters())/1e6:.2f}M")
...
P(f"v24 val: {len(val_texts)} samples")
...
P(f"*** v28.5 PPL: {ppl:.3f} ***")
P(f"    对比 v25: 2.44 ({(ppl-2.44)/2.44*100:+.1f}%)")
P(f"    对比 v28: 42.26 ({(ppl-42.26)/42.26*100:+.1f}%)")


# ===== 速度 =====
@torch.no_grad()
def gen_v28_5(n_ar=100):
...
P("\n=== 速度 ===")
t = bench(gen_v28_5, label="v28.5 AR (no KV cache)")
P(f"\nv28.5 速度: {t:.0f} ms (vs v25 502ms, ratio {t/502:.2f}x)")

# 质量
cur = gen_v28_5(n_ar=100)
P(f"\n生成内容: {''.join([itos.get(t, '?') for t in cur[:80]])}")

results = {"ppl_v28_5": ppl, "speed_ms": t, "speedup_vs_v25": t/502, "params_M": sum(p.numel() for p in decoder.parameters())/1e6}
with open("v28_5_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2)
P(f"\nSaved: v28_5_results.json")