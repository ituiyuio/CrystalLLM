...
"""
eval_v20a_e2e.py — v20a 端到端评估

v20a 229M decoder + 复用 v19 prior + 复用 v18 encoder (cached z).
三模式: diffusion_z / encoder_mu / random_z
PPL: 全 val 集 (210 样本), 与 baseline / v19 同口径对比
"""
...
# ===== v20a decoder 加载 =====
ckpt_dec = torch.load("crystalllm/proto_v20a_decoder.pt", map_location="cuda", weights_only=False)
cfg = ckpt_dec["config"]
T, D_Z = cfg["T"], cfg["D_Z"]
DEC_LAYER, DEC_HEAD, DEC_EMBD = cfg["DEC_LAYER"], cfg["DEC_HEAD"], cfg["DEC_EMBD"]
print(f"v20a decoder: {DEC_LAYER}L × {DEC_EMBD} × {DEC_HEAD}")
...
print(f"v20a decoder params: {n_dec/1e6:.2f}M")
...
print(f"\n=== 对照 (跨版本) ===")
print(f"  v18 (87M):     val_recon 2.78 (encoder_mu PPL 16.2 / e2e 17.7)")
print(f"  v20a (229M):   encoder_mu {ppl_enc:.2f} / e2e {ppl_diff:.2f}")
...
with open("crystalllm/v20a_e2e.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2)
print(f"\nSaved: crystalllm/v20a_e2e.json")
