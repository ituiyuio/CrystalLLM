...
"""
debug_v29_real_accept.py — 测试真正的接受率
drafter 输出 tokens, verifier 也看这些 tokens, 比较 argmax
"""
...
for trial in range(10):
    ...
    with torch.no_grad():
        ...
        d_logits = drafter.head(x_t)
        d_tokens = d_logits.argmax(dim=-1)  # (1, N)

        # verifier 看 d_tokens
        x = d_tokens  # (1, N)
        v_logits = verifier(z, x)  # (1, N, V)
        v_tokens = v_logits.argmax(dim=-1)  # (1, N)

        # 逐位置接受
        n_acc = 0
        for j in range(N):
            d_t = d_tokens[0, j].item()
            v_t = v_tokens[0, j].item()
            if d_t == v_t:
                n_acc += 1
            else:
                break
        ...
        print(f"  trial {trial}: drafter={d_tokens[0,:8].cpu().tolist()}, verifier={v_tokens[0,:8].cpu().tolist()}, acc={n_acc}/{N}")

print(f"\n总体接受率: {n_acc_total}/{n_drafted_total} = {n_acc_total/n_drafted_total*100:.1f}%")