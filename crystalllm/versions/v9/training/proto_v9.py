...
"""
proto_v9.py — 扩规模到 50M 参数 (16 层 × 512 embd)

目标: 验证 prefix-LM 设计在更大规模下仍能保持 PPL + z 空间结构
复用: v8 的所有设计 (smart gen init, 完整 prefix-LM, 三损失)

配置:
  N_LAYER=16, N_EMBD=512, N_HEAD=8  ≈ 52M 参数
  STEPS=3000 (与 v6/v8 一致, 不变)
  训练: 同 v8 (无 mask)
  gen:  同 v8 (real starter init)

评估:
  ① val PPL — 与 v6 (12M) / v8 (12M) 对比, 看 scaling 趋势
  ② z 空间 — 看 mpd / PCA / effective rank 如何随规模演化
  ③ 纯扩散生成 — 看更大模型能否生成更结构化文本
"""
...
B, T, D_Z      = 32, 256, 64
T_HALF         = T // 2
N_LAYER, N_HEAD, N_EMBD = 16, 8, 512                   # 16×512 ≈ 52M
LR, STEPS      = 3e-4, 3000
EVAL_EVERY     = 500
...
print(f"\n=== train {STEPS} steps (50M, same loss as v8) ===")
t0 = time.time()
log = []
...
ax.set_title(f'v9 z space (50M, 1317 sessions, prefix-LM)\n'
             f'‖z‖ {norms.min():.1f}–{norms.max():.1f}, mpd {mpd:.2f}')
ax.grid(True, alpha=0.3)
plt.tight_layout()
plt.savefig('crystalllm/v9_z_space.png', dpi=100, bbox_inches='tight')
print("Plot saved: crystalllm/v9_z_space.png")

# 评估 ② z 插值生成
print("\n=== z interpolation (v9 real starter) ===")
...
# 评估 ③ 纯扩散生成
print("\n=== pure diffusion generation (z ~ N(0,I)) ===")
...
# 评估 ④ seed-based gen (不同 seed 风格)
print("\n=== seed-based gen (real starter) ===")
for seed in ["def ", "class ", "import ", "# ", "the ", "## "]:
    out = model.gen(seed, n=150, use_real_starter=True)
    print(f"  seed={seed!r}: {out[:200]!r}\n")

# 保存模型
SAVE_PATH = "crystalllm/proto_v9_model.pt"
