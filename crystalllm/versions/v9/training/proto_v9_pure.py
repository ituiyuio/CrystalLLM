...
"""
proto_v9_pure.py — 纯 AR 50M baseline (no z, no diffusion)

架构: 与 v9/v10/v11 完全一致, 仅删除 z_enc/z_dec/z_to_chars/diffusion 模块
目的: 公平对比. 在 50M 参数 + 同数据下, 看 z 和扩散是否真带来收益.
"""
...
print(f"Model: {n_params/1e6:.2f}M params (pure AR, no z)  |  device: {DEVICE}")
...
for step in range(STEPS):
    ...
    if step % EVAL_EVERY == 0 or step == STEPS - 1:
        ...
        print(f"  step {step:4d} | train {loss.item():.3f} | val {vloss.item():.3f} "
              f"| val_ppl {math.exp(vloss.item()):.1f} | {time.time()-t0:.0f}s")
...
# 保存
SAVE_PATH = "crystalllm/proto_v9_pure_model.pt"
