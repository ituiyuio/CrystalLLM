"""
eval_v33_hybrid_lite.py — v33-D Lite Hybrid 评估
"""
...
P("=== v33-D Lite Hybrid AR-Diffusion 评估 ===")
...
# ===== v33 Lite Drafter =====
ckpt_d = torch.load("v33_hybrid_lite.pt", map_location="cuda", weights_only=False)
dcfg = ckpt_d["config"]
...
drafter = HybridDrafterLite().to("cuda")
drafter.load_state_dict(ckpt_d["model"])
drafter.eval()
tok_emb = nn.Embedding(V, D_EMB).to("cuda")
tok_emb.load_state_dict(ckpt_d["tok_emb"])
tok_emb.eval()
P(f"v33 Lite Drafter: {sum(p.numel() for p in drafter.parameters())/1e6:.2f}M (1 refine block)")
...
@torch.no_grad()
def sample_K_tokens_lite(z, K=8, n_steps=5):
    """ODE 5 步 + 最后 refine 1 次"""
    x_t = torch.randn(1, K, D_EMB, device="cuda")
    dt = 1.0 / n_steps
    for k in range(n_steps):
        t_val = k * dt
        ...
        x_t = x_t + dt * v
    # 最后 refine 1 次
    t_final = torch.full((1,), 1.0, device="cuda")
    x_t = drafter.refine(x_t, z, t_final)
    logits = F.linear(x_t, tok_emb.weight)
    return logits.argmax(dim=-1)[0]


@torch.no_grad()
def gen_v33_lite_sps(z, n_ar=100, K=8, mode="lite"):
    cur = [BOS_ID]
    n_rounds = 0
    n_total_drafted = 0
    n_total_accepted = 0
    ...
    while len(cur) - 1 < n_ar:
        n_rounds += 1
        if mode == "lite":
            draft = sample_K_tokens_lite(z, K=K)
        else:
...
def gen_ode_only():
    return gen_v33_lite_sps(z_test, n_ar=100, K=8, mode="ode_only")

def gen_lite():
    return gen_v33_lite_sps(z_test, n_ar=100, K=8, mode="lite")

t_ode = bench(gen_ode_only, label="v33 Lite ODE-only")
t_lite = bench(gen_lite, label="v33 Lite ODE+Refine(1)")

P(f"\n速度对比:")
P(f"  v33 Lite ODE-only: {t_ode:.0f} ms")
P(f"  v33 Lite ODE+Refine(1): {t_lite:.0f} ms")
P(f"  v31 baseline: 206 ms")
P(f"  v33-C (3 refine): 560 ms")
P(f"  v33 Lite 加速比 vs v31: {206/t_lite:.2f}x")
...
for trial in range(10):
    cur, n_rounds, drafted, accepted = gen_v33_lite_sps(z_test, n_ar=100, K=8, mode="lite")
    total_d += drafted; total_a += accepted
    n_rounds_avg += n_rounds
acc_rate = total_a / total_d if total_d > 0 else 0
n_rounds_avg /= 10
P(f"  v33 Lite: {total_a}/{total_d} = {acc_rate*100:.1f}%, 平均 rounds={n_rounds_avg:.1f}")

# 生成质量
P("\n=== 生成质量 ===")
cur_lite, _, _, _ = gen_v33_lite_sps(z_test, n_ar=100, K=8, mode="lite")
P(f"v33 Lite: {repr(''.join([itos.get(t, '?') for t in cur_lite[:60]]))}")

# 速度分解
P("\n=== 速度分解 ===")
def time_drafter_lite():
    return sample_K_tokens_lite(z_test, K=8)
...
for _ in range(10):
    ...
    times.append((time.time() - t0) * 1000)
P(f"  drafter ODE-only (Lite, K=8): {np.mean(times):.2f} ms")

times = []
for _ in range(10):
    torch.cuda.synchronize(); t0 = time.time(); time_drafter_lite(); torch.cuda.synchronize()
    times.append((time.time() - t0) * 1000)
P(f"  drafter ODE+Refine(1) (Lite, K=8): {np.mean(times):.2f} ms")

results = {
    "v33_lite_ode_only_ms": t_ode,
    "v33_lite_hybrid_ms": t_lite,
    "speedup_vs_v31": 206 / t_lite,
    "speedup_vs_v33_c": 560 / t_lite,
    ...
    "v31_baseline_ms": 206,
    "v33_c_baseline_ms": 560
}
with open("v33_lite_results.json", "w", encoding="utf-8") as f:
    json.dump(results, f, indent=2)
P(f"\nSaved: v33_lite_results.json")