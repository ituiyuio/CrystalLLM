"""
proto_v21_500m_decoder.py — v21: 500M BAD decoder

从 v20a 229M 扩到 v21 500M (24L × 1280 × 20).
Encoder 不动, 复用 v18 cached z.

架构:
  Encoder (frozen, 12L×768×12, 87M) — cached z 已就绪
  Decoder (NEW, 24L×1280×20, ~500M) — 主训练对象

训练目标: 同 v20a BAD-DP
  L = 1.0 * L_recon + 0.1 * beta * L_KL(q(z|x) || N(0,I))

预期: val_recon PPL < v20a (12.54), 端到端 (with v19 prior) PPL 接近 11.5 baseline.
"""
...
P("=== v21 500M BAD Decoder STARTUP ===")
...
# ===== 配置 =====
B, T, D_Z = 16, 128, 64  # 与 v18/v20a 一致
# Decoder 扩到 24L × 1280 × 20 (目标 ~500M)
DEC_LAYER, DEC_HEAD, DEC_EMBD = 24, 20, 1280
LR, STEPS = 1.5e-4, 4000   # 比 v20a (2e-4) 再降一档, 500M 更稳
EVAL_EVERY = 250
...
class Decoder(nn.Module):
    """v21 大 decoder: 24L × 1280 × 20. 同 v18 BAD 接口 (decoder 只看 z)."""
    def __init__(s):
        super().__init__()
...
P(f"对比 v20a decoder: 229M (18L × 1024 × 16)")
...
for step in range(STEPS):
    ...
    logvar = torch.full_like(z, -3.0)  # 模拟 encoder 高 confidence (用 cache mu)
    logits = decoder(z, x)
...
SAVE = "crystalllm/proto_v21_decoder.pt"
...
with open("crystalllm/v21_train_log.json", "w", encoding="utf-8") as f:
    json.dump({"log": log, "config": {"STEPS": STEPS, "W_KL": W_KL, "W_RECON": W_RECON,
                                        ...
                                        "arch": "v21-500M-BAD-decoder"}}, f, indent=2)
P(f"Log saved: crystalllm/v21_train_log.json")
