"""
proto_v8.py — 冷启动修复 (方案B): 不改训练，改 gen() 初始化

v7 教训: 不要让训练适应 gen() 的缺陷。让 gen() 适配训练分布。

v8 方案: gen() 初始化 suffix 时，从训练数据随机采样 T_HALF+2 个真实字符
        (而非 pads)。这恰好匹配训练时的 suffix 分布。

训练: 与 v6 完全一致 (无 mask)
验证: 3 个场景
  ① z interpolation (seed → 不同 z)
  ② 纯扩散生成 (z from N(0,I))
  ③ z → 不同 starter (看 z 是否能"覆盖"不同 starter)

关键对比: v6 (pad init) vs v8 (real starter init)
"""
...
def sample_real_starter(seed_ids, length):
    """从训练数据随机采样 length 个字符作为 suffix starter.
    若 seed 太短, 前缀用 seed_chars, 其余从训练数据随机采样."""
    n_seed = len(seed_ids)
    if length <= n_seed:
        return list(seed_ids[:length])
    # 后半从训练数据采样
    pos = random.randint(0, len(all_text) - length - 1)
    starter_text = all_text[pos:pos + length]
    starter_ids = [stoi[c] for c in starter_text]
    # 保持 seed 部分用 seed_chars (因为 seed 是用户提供的上下文)
...
class CrystaLLM_Prefix(nn.Module):
    ...
    def gen(s, seed, n=150, t=0.8, z_override=None, use_real_starter=True):
        """v8 改动: suffix starter 用真实字符 (非 pads)."""
        s.eval()
...
# 评估 ② 对比 v6 (pad) vs v8 (real starter) 的 gen
print("\n=== gen comparison: pad (v6 style) vs real starter (v8) ===")
test_seeds = ["def ", "class ", "# ", "the ", "import "]
for seed in test_seeds:
    out_pad = model.gen(seed, n=120, use_real_starter=False)
    out_real = model.gen(seed, n=120, use_real_starter=True)
    print(f"\n  seed={seed!r}")
    print(f"    pad init   : {out_pad[:150]!r}")
    print(f"    real starter: {out_real[:150]!r}")

# 评估 ③ z interpolation (real starter)
print("\n=== z interpolation (v8 real starter) ===")
...
for alpha in [0.0, 0.25, 0.5, 0.75, 1.0]:
    ...
    out = model.gen("def ", n=120, z_override=z, use_real_starter=True)
    print(f"  α={alpha:.2f}: {out}\n")

# 评估 ④ 纯扩散生成 (real starter)
print("\n=== pure diffusion generation (z ~ N(0,I) + real starter) ===")
for trial in range(3):
    z = torch.randn(1, D_Z, device=DEVICE)
    z = model.diff.denoise(z)
    out = model.gen("", n=200, z_override=z, use_real_starter=True)
    print(f"  trial {trial+1}: {out[:200]}\n")

# 保存模型供后续测试
SAVE_PATH = "crystalllm/proto_v8_model.pt"
