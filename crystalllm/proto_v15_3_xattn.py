"""
proto_v15_3_xattn.py — v15.3: prefix 注入 (v14 风格) + cross-attn decoder

修复方向:
- 移除独立 encoder 块 (避免 encoder 过拟合坍缩)
- 用 v14 的 prefix-position-0 注入 z (已知稳定)
- decoder 加入 cross-attn 块 (验证 cross-attn 单独效果)
- 规模回到 ~240M (16L × 1024 embd, 类似 v12)

架构:
  Encode: prefix → tok_emb + pos_emb → 16 × BlockPure → mean pool → z_enc → z
  Decode: [z_emb(pos 0)] + [sfx_emb(pos 1..T/2+1)]
          → 16 × BlockXattn (self-attn + cross-attn to z + MLP)
          → ln_f → head

主题损失: L = L_pred + 0.4·L_recon + 0.05·L_diff + 0.1·L_theme (v14 一致)
"""
...
# v15.3 配置: ~240M (与 v12 类似), v14 的 prefix 注入 + cross-attn decoder
B, T, D_Z      = 24, 256, 64
T_HALF         = T // 2
N_LAYER, N_HEAD, N_EMBD = 16, 16, 1024  # ~240M
LR, STEPS      = 3e-4, 2000
EVAL_EVERY     = 250
W_PRED, W_RECON, W_DIFF, W_THEME = 1.0, 0.4, 0.05, 0.1  # 与 v14 一致
...
class BlockXattn(nn.Module):
    """v15.3 Decoder block: self-attn + cross-attn to z + MLP."""
...
class BlockPure(nn.Module):
    """v15.3 共享 block: 用于 encode (纯 self-attn)."""
...
class ControllableHybridV3(nn.Module):
    """v15.3: prefix 第 0 注入 z (v14 风格) + cross-attn decoder blocks.

    Encode path: prefix → tok+pos → 16 × BlockPure → mean pool → z
    Decode path: [z_emb @ pos 0] + [tok+pos @ pos 1..] → 16 × BlockXattn → head
    """
    def __init__(s):
        super().__init__()
        s.tok = nn.Embedding(V, N_EMBD); s.pos = nn.Embedding(T, N_EMBD)
        # 共享 encoder blocks (与 v9 一致)
        s.enc_blocks = nn.ModuleList([BlockPure(N_EMBD, N_HEAD) for _ in range(N_LAYER)])
        # decoder blocks (cross-attn)
        ...
        s.z_dec = nn.Linear(D_Z, N_EMBD)  # v14 风格: prefix pos 0 注入
    ...
    def decode(s, z, suffix):
        B_, T_s = suffix.shape
        z_emb = s.z_dec(z).unsqueeze(1)  # v14: prefix 第 0 位置注入
...
model = ControllableHybridV3().to(DEVICE)
n_params = sum(p.numel() for p in model.parameters())
print(f"\nModel v15.3: {n_params/1e6:.2f}M params")
print(f"  Encoder: {N_LAYER} × BlockPure | Decoder: {N_LAYER} × BlockXattn (cross-attn)")
print(f"  z 注入: prefix pos 0 (v14 style) + cross-attn in decoder")
...
print(f"  z 范数: mean={z_norms.mean():.2f}, std={z_norms.std():.2f}, min={z_norms.min():.2f}, max={z_norms.max():.2f}")
...
for text, theme in demo_items:
    ...
    print(f"    edit:   {safe(out_edit[20:80])}")
...
# 保存
SAVE_PATH = "crystalllm/proto_v15_3_xattn_model.pt"
...
out_json = {"log": log, "val_theme_acc": float(acc), "per_theme_acc": per_theme,
            ...
            "config": {"STEPS": STEPS, "W_THEME": W_THEME, "N_LAYER": N_LAYER, "N_EMBD": N_EMBD,
                       ...
                       "arch": "v15.3-prefix-inject+xattn-decoder"}}
with open("crystalllm/v15_3_train_log.json", "w", encoding="utf-8") as f:
    json.dump(out_json, f, indent=2, ensure_ascii=False)
print(f"Log saved: crystalllm/v15_3_train_log.json")
print(f"\n=== v15.3 训练完成 ({time.time()-t0:.0f}s) ===")