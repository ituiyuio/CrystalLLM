"""Exp 12 (CMT ablation Fix-3): cmt_liere_real_cayley - M3 LieRE 伪 Cayley 修复.

修复内容:
  - 替换 LieRE_Cayley (block-diagonal 2D 旋转 + context_net) → LieRE_RealCayley
  - 修复 M3: 实现真正的 Cayley 变换 R = (I-A)^{-1}(I+A), 完整 SO(n)
  - 风险: O(d^3) 矩阵求逆, d=640 单步可能较慢; 失败时自动降级 d_model=384

架构: Embedding -> N x CMTBlockV2(LieRE_RealCayley + WaveAttention + ComplexKANFFN_Full) -> LN -> Head

参考文档:
  - docs/notes/2026-06-21-wave-function-scalpel.md §📐 M3
  - docs/superpowers/specs/2026-06-21-cmt-ablation-fix-design.md §3.3
"""
import argparse
import json
import math
...
class CMT50M_Fix3(nn.Module):
    """50M CMT 模型, pe 替换为 LieRE_RealCayley (真 Cayley 变换)."""
...
def build_cmt_fix3_50m(**kwargs):
    return CMT50M_Fix3(**kwargs)
...
def main():
    ...
    print(f"=== Exp 12: Fix-3 (LieRE_RealCayley) - M3 LieRE 真 Cayley 修复 ===")
    ...
    print(f"警告: 真 Cayley 需要 O(d^3) 矩阵求逆, d=640 单步可能 5-10s\n")

    model = build_cmt_fix3_50m(
        d_model=args.d_model, n_layers=args.n_layers,
        n_heads=args.n_heads, kan_dim=args.kan_dim,
    )
    n_params = count_active_params(model)
    print(f"CMT-Fix3 模型参数数: {n_params:,}")
    ...
    result = {
        "exp_id": "exp12_cmt_liere_real_cayley",
        "fix_target": "M3 (LieRE 伪 Cayley)",
        "config": vars(args),
        "n_params": n_params,
    ...
    if val_ppls:
        ...
        print(f"\n=== 与 Exp 8 对比 ===")
        print(f"  Exp 8 (伪 Cayley):         32.58")
        print(f"  Exp 12 (真 Cayley):        {final_ppl:.4f}")
        if final_ppl < 28:
            print(f"  [OK] PPL < 28, Fix-3 有效")
        elif final_ppl < 30:
            print(f"  [PARTIAL] 有所改善")
        else:
            print(f"  [X] PPL ≥ 30, Fix-3 无效, M3 非主要失败机制")
