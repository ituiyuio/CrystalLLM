"""Exp 14 (CMT ablation Fix-5): cmt_no_context_pe - M3 简化 (去除 context_net).

修复内容:
  - 替换 LieRE_Cayley (block-diagonal 2D 旋转 + context_net) → LieRE_NoContext
  - 验证 M3 是否纯粹是 "context_net 训练无信号" 问题
  - 与 Fix-3 (真 Cayley) 形成对比: 简化版 PE vs 完整 SO(n) PE

架构: Embedding -> N x CMTBlockV2(LieRE_NoContext + WaveAttention + ComplexKANFFN_Full) -> LN -> Head

参考文档:
  - docs/notes/2026-06-21-wave-function-scalpel.md §📐 M3 简化
  - docs/superpowers/specs/2026-06-21-cmt-ablation-fix-design.md §3.5
"""
import argparse
import json
import math
...
class CMT50M_Fix5(nn.Module):
    """50M CMT 模型, pe 替换为 LieRE_NoContext (标准 RoPE 风格)."""

    def __init__(self, vocab_size=VOCAB_SIZE, d_model=640,
                 n_layers=8, n_heads=8, kan_dim=96,
...
def build_cmt_fix5_50m(**kwargs):
    return CMT50M_Fix5(**kwargs)
...
def main():
    ...
    print(f"=== Exp 14: Fix-5 (LieRE_NoContext) - M3 简化 (去除 context_net) ===")
    ...
    print(f"用标准 RoPE 风格 2D 旋转, 不依赖 context_net\n")

    model = build_cmt_fix5_50m(
        d_model=args.d_model, n_layers=args.n_layers,
        n_heads=args.n_heads, kan_dim=args.kan_dim,
    )
    n_params = count_active_params(model)
    print(f"CMT-Fix5 模型参数数: {n_params:,}")
    ...
    result = {
        "exp_id": "exp14_cmt_no_context_pe",
        "fix_target": "M3 简化 (去除 context_net)",
        "config": vars(args),
        "n_params": n_params,
    ...
    if val_ppls:
        ...
        print(f"\n=== 与 Exp 8 对比 ===")
        print(f"  Exp 8 (LieRE context):     32.58")
        print(f"  Exp 14 (LieRE no-context): {final_ppl:.4f}")
        if final_ppl < 25:
            print(f"  [OK] PPL < 25, M3 主要是 context_net 问题, 简化版有效")
        elif final_ppl < 30:
            print(f"  [PARTIAL] 有所改善")
        else:
            print(f"  [X] PPL ≥ 30, 即便简化 PE 也无效, CMT 整体架构问题")
