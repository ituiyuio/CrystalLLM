...
class CMT50M_Fix6(nn.Module):
    """50M CMT 模型 v2: 三刀同步 (Fix-1+2+3 组合)."""
...
def build_cmt_fix6_50m(**kwargs):
    return CMT50M_Fix6(**kwargs)
...
def main():
    ...
    print(f"CMT-Fix6 模型参数数: {n_params:,}")
    ...
    result = {
        "exp_id": "exp15_cmt_full_v2",
        "fix_target": "三刀同步 v2 (Fix-1+2+3 组合)",
        "config": vars(args),
        "n_params": n_params,
