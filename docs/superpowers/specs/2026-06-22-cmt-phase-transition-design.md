...
| Step | V49 50M val_ppl | V49 50M diversity | 判定 |
|------|-----------------|-------------------|------|
| 4k | 2.99 (2k=4.97) | 0.135 (5k) | 应是真 LM 状态 |

**目的**: 验证"n-gram entropy" / "top-1 confidence" / "val-train gap" 在已知真 LM 上的数值范围, 为 CMT 检查点的真 LM 判定提供校准基线.
...
1. val_ppl ∈ [1.5, 3.0]
2. diversity ≥ 0.3
3. n-gram entropy ≥ 1.0 bit
4. val-train PPL gap > 0.1

**Memorizer 状态**:
- val_ppl < 1.5 AND diversity < 0.3 AND entropy < 0.5 bit AND gap < 0.05

**欠拟合状态**: 其余 (val_ppl > 3.0 或多样性低但 entropy 高)
...
本实验**修补 v1.0 标准的盲区**. 实验结束后**更新 v1.0 标准**, 添加这 3 个新维度作为 Phase-2 评估的强制项:
